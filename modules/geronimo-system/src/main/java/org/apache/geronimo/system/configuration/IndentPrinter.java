    ...
    public void printSpace()
    {
        ...
        if ( _text.length() > 0 ) {
            ...
            if ( format.getLineWidth() > 0 &&
                 _thisIndent + _line.length() + _spaces + _text.length() > format.getLineWidth() ) {
                flushLine( false );
                try {
    ...
    public void flushLine( boolean preserveSpace )
    {
        ...
        if ( _line.length() > 0 ) {
            try {
                ...
                if ( format.getIndenting() && ! preserveSpace ) {
                    // Make sure the indentation does not blow us away.
                    indent = _thisIndent;
                    if ( ( 2 * indent ) > format.getLineWidth() && format.getLineWidth() > 0 )
                        indent = format.getLineWidth() / 2;
