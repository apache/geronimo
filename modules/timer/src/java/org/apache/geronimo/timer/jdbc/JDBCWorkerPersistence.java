...
public class JDBCWorkerPersistence implements WorkerPersistence, GBeanLifecycle {
    ...
    private static final String createTableSQL = "create table timertasks (id long primary key, serverid varchar(256) not null, timerkey varchar(256) not null, userinfo varchar(4096), firsttime long not null, period long, atfixedrate boolean not null)";
    private static final String sequenceSQL = "select timertasks_seq.nextval";
    private static final String insertSQL = "insert into timertasks (id, serverid, timerkey, userinfo, firsttime, period, atfixedrate) values (?, ?, ?, ?, ?, ?, ?)";
    private static final String deleteSQL = "delete from timertasks where id=?";
    private static final String selectSQL = "select id, userinfo, firsttime, period, atfixedrate from timertasks where serverid = ? and timerkey=?";
